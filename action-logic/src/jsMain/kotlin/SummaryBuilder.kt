...
object SummaryBuilder {
    ...
    fun buildDiff(path: String, existingSizeBytes: Long, newSizeBytes: Long): String {
        return buildString {
            ...
            val formattedDiff = formatSize(diffBytes)
            ...
            appendLine()
            appendLine("<sup>_Generated by `jacobras/size-diff-action`_</sup>")
        }
    }
    ...
    private fun formatSize(bytes: Long): String {
        return when {
            ...
            bytes < 1024 * 1024 -> "${(bytes.toDouble() / 1024).toFixed(0)} kB"
            bytes < 1024 * 1024 * 1024 -> "${(bytes.toDouble() / 1024 / 1024).toFixed(1)} MB"
            else -> "unknown"
        }
    }
}